//
//  GlobalMessageServiceCoreDataHelper.swift
//  GMS Worldwide App
//
...
/**
 Easy `CoreData`.
 */
public final class GlobalMessageServiceCoreDataHelper {
  ...
  private lazy var managedObjectModel: NSManagedObjectModel = {
    ...
    let bundle = GlobalMessageService.bundle
  ...
  private lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator = {
    ...
    do {
      ...
      gmsLog.error("Unresolved error \(wrappedError), \(wrappedError.userInfo)")
    }
...
internal extension GlobalMessageServiceCoreDataHelper {
  
  /// `String` with .xcdatamodeld filename (without extension)
  private static var modelName: String = "GlobalMessageServicesInbox"
  ...
  /**
   ...
   `GlobalMessageServiceCoreDataHelper
  ...
  internal static func initializeSharedInstance(withModelName modelName: String, fileName: String) {
    GlobalMessageServiceCoreDataHelper.modelName = modelName
    GlobalMessageServiceCoreDataHelper.fileName = fileName
  }
    ...
    managedObjectContext.performBlockAndWait {
      if self.managedObjectContext.hasChanges {
        do {
          ...
          gmsLog.error("Unresolved error \(nserror), \(nserror.userInfo)")
        }
      }
    }
  }
...
// MARK: - Static
extension GlobalMessageServiceCoreDataHelper {
  
  /// The shared `GlobalMessageServiceCoreDataHelper` object for the process. (read-only)
  private (set) static var sharedInstance: GlobalMessageServiceCoreDataHelper = {
    return GlobalMessageServiceCoreDataHelper(withModelName: modelName, fileName: fileName)
  }()
